...
<template>
  <div
    class="group relative bg-card/40 backdrop-blur-xl border-2 border-white/5 rounded-2xl hover:border-primary/30 transition-all duration-500 hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] animate-in fade-in slide-in-from-bottom-4">
    <!-- Header: Duelist Info -->
    <div class="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
      <div class="flex items-center gap-3">
        <div
          class="relative w-10 h-10 rounded-full border border-white/10 p-0.5 group-hover:border-primary/50 transition-colors">
          <div class="w-full h-full rounded-full bg-muted/20 flex items-center justify-center overflow-hidden">
            <User class="w-5 h-5 text-muted-foreground" />
          </div>
          <div
            class="absolute -bottom-1 -right-1 w-4 h-4 bg-primary rounded-full border-2 border-background flex items-center justify-center">
            <Sparkles class="w-2 h-2 text-primary-foreground" />
          </div>
        </div>
        <div>
          <p class="text-xs font-black uppercase tracking-widest text-white italic truncate max-w-[120px]">{{ isOwn ?
            'Sua Oferta' : trade.user.name }}</p>
      ...
      <Button v-if="isOwn" variant="holographic" size="sm"
        class="h-8 w-8 p-0 rounded-lg border-destructive/20 hover:bg-destructive/10 hover:text-destructive"
        @click="emit('delete', trade.id)">
        <Trash2 class="w-4 h-4" />
      </Button>
      <div v-else
        class="px-2 py-0.5 rounded bg-primary/10 border border-primary/20 text-[8px] font-black uppercase tracking-widest text-primary animate-pulse">
        Ativa
      </div>
    </div>
    ...
    <div class="p-5 flex flex-col gap-6 relative">
      ...
      <div class="absolute inset-0 bg-gradient-to-tr from-primary/5 via-transparent to-transparent pointer-events-none">
      </div>
      ...
      <div class="flex items-center justify-center py-1">
        <div class="relative">
          ...
          <div
            class="relative w-10 h-10 rounded-full border-2 border-primary/30 flex items-center justify-center bg-background shadow-2xl">
